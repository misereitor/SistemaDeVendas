...
namespace SistemaDeVendas.Data.Map
{
    public class ParametrosDeVendasModelMap : IEntityTypeConfiguration<ParametrosdeVendasModel>
    {
        public void Configure(EntityTypeBuilder<ParametrosdeVendasModel> builder)
        {
            ...
            builder.Property(p => p.FechamentoCegoPDV)
                .HasDefaultValue(false);

            builder.Property(p => p.LimiteNumerarioCaixa);

            builder.Property(p => p.CupomTroca)
                .HasDefaultValue(false);

            builder.Property(p => p.PrasoCupomTroca);

            builder.Property(p => p.BloqVendaEstoqueNegativo)
                .HasDefaultValue(false);

            builder.Property(p => p.BloqProducaoEstoqueNegativo)
                .HasDefaultValue(false);

            builder.Property(p => p.MovimentacaoEstoqViaXML)
                .HasDefaultValue(false);

            builder.Property(p => p.CadastroDeProdutoViaPedidoCompra)
                .HasDefaultValue(false);

            builder.Property(p => p.CadastroDeProdutoViaPedidoVenda)
                .HasDefaultValue(false);
